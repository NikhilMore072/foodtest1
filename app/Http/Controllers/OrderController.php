...
class OrderController extends Controller
{
    ...
    // {
    //     $data['my_orders'] = ClinicOrders::select(
    //         'order_id',
    //         DB::raw('MIN(clinic_orders.created_at) AS earliest_created_at'),
    //         DB::raw('SUM(clinic_orders.total_price) AS total_price'),
    //         DB::raw('MIN(clinic_orders.order_status) AS max_order_status'),
    //         DB::raw('MIN(clinic_orders.received_remarks) AS max_received_remarks'),
    //         DB::raw('GROUP_CONCAT(DISTINCT product_name.name) AS product_name'),
    //         DB::raw('GROUP_CONCAT(DISTINCT clinic_orders.product_qty) AS product_qty'),
    //         DB::raw('CONCAT(clinic_location.branch_name, ",", clinic_location.location) AS clinic_name'),
    ...
    public function receivedOrders(Request $request)
    {
        $data['my_orders'] = ClinicOrders::select(
            'clinic_orders.id',
            'clinic_orders.order_id',
            'clinic_orders.created_at',
            'total_price',
            'order_status',
            'received_remarks',
            DB::raw('GROUP_CONCAT(product_name.name) AS product_name'),
            DB::raw('GROUP_CONCAT(clinic_orders.product_qty) AS product_qty'),
            DB::raw('CONCAT(clinic_location.branch_name,",",clinic_location.location) AS clinic_name'),
            'manufacturer.name AS mn_name',
            'category.category_name'
        )
            ...
            ->groupBy(
                ...
                'manufacturer.name',
                'category.category_name',
                'clinic_location.branch_name',
                'clinic_location.location'
            )
            ->orderBy('clinic_orders.created_at','DESC')
            ->get();
        $data['my_orders'] = $data['my_orders']->unique('order_id')->values()->all();
    ...
    public function completedOrder(Request $request)
    {
        $data['my_orders'] = ClinicOrders::select(
            ...
            ->leftJoin('manufacturer', 'manufacturer.id', '=', 'clinic_orders.manfracture_id')
            ->leftJoin('category', 'category.id', '=', 'clinic_orders.category_id')
            ->where('clinic_orders.order_status', 1)
            ->groupBy(
                'clinic_orders.id',
                'order_id',
                'clinic_orders.created_at',
            ...
            ->orderBy('clinic_orders.updated_at','DESC')
            ->get();
