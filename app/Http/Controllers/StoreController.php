...
class StoreController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = Store::select(DB::raw('GROUP_CONCAT(store.id) AS ids'),
        DB::raw('GROUP_CONCAT(store.barcode_id) AS group_barcode_id'),
        DB::raw('GROUP_CONCAT(store.qty) AS group_qty'),
        DB::raw('SUM(store.qty) AS total_qty'),'c.category_name','pn.name AS pr_name','mn.name AS mn_name','v.vendor_name')
        ...
        ->leftJoin('vendors AS v','v.id','=','store.vendor_id')
        ->where('store.qty','>',0)
        ->groupBy('store.product_name')->get();      
...
// }
    ...
    public function add_stock(Request $requst)
    {
        ...
        $vendr=Vendors::Where('status','active')->get();
        return view('stock', compact('category', 'manu', 'product_name', 'unit','vendr'));
    }

    public function edit_stock(Request $request)
    {
        $id = $request->id;
        $data = Store::with('Category_model', 'Manufacturer_model', 'Product_model','Vendor_model')->Where(['user_id' => Auth::user()->id, 'id' => $id])->first();
        $category = Category::Where('is_active', '1')->get();
        $manu = Manufacturer::Where('is_active', '1')->get();
        $product_name = Product_name::Where('is_active', '1')->get();
        $unit = Unit::Where('is_active', '1')->get();
        ...
        return view('edit_stock', compact('data', 'category', 'manu', 'product_name', 'unit','vendr'));
    }

    protected function stock_register(Request $request)
    {
        // For old code check git file Date:30-03-2023
        $vid=$request['vendor_name'];
    ...
    protected function update_stock(Request $request)
{
    $update = Store::where('id', $request->input('tabel_id'))->update(['is_active' => 0]);

    $data = [
        'user_id' => $request->input('user_id'),
        'category' => ucfirst(strtolower($request->input('category'))),
        'manufacture_name' => ucfirst(strtolower($request->input('manufacture_name'))),
        'product_name' => ucfirst(strtolower($request->input('product_name'))),
    ...
    $result = Store::where('id', $request->input('tabel_id'))->update($data);
          session()->flash('message', 'Product Update Successfully!');
        session()->flash('alert-class', 'alert-success');
        return Redirect::to('/store-eq')->with(['status' =>  'success']);
