...
class DispatchController extends Controller
{
    ...
    // {
    //     $data['dispatch_data'] = TBLDispatch::select(
    //             'tbl_dispatch.*',
    //             'cl.branch_name',
    //             'cl.location'
    //         )
    //         ->leftJoin('clinic_location AS cl', 'cl.id', '=', 'tbl_dispatch.clinic_id')
    //         ->where('tbl_dispatch.accepted', 0) 
    //         ->groupBy(
    //             'tbl_dispatch.id',
    //             'order_id',
    //             'cl.branch_name',
    ...
    public function index()
{
    $data['dispatch_data'] = TBLDispatch::select(
        ...
        ->leftJoin('clinic_location AS cl', 'cl.id', '=', 'tbl_dispatch.clinic_id')
        ->where('tbl_dispatch.is_accepted', 0) 
        ->groupBy('order_id')
    ...
    public function all_dispatch()
    {
        $data['dispatch_data'] = TBLDispatch::select(
            ...
            ->orderBy('tbl_dispatch.created_at', 'DESC') 
            ->get();
    ...
    {       
        $data['store_list'] = Clinic_location::select()->where('is_active',0)->where('user_id',Auth::user()->id)->get();
        $data['order_details'] = ClinicOrders::select('clinic_orders.id','order_id','clinic_orders.created_at','total_price','order_status','received_remarks','pn.name','clinic_orders.product_qty','clinic_orders.product_unit','clinic_orders.price_per_unit','clinic_id','cl.branch_name','clinic_orders.product_id')
        ->leftJoin('clinic_location AS cl','cl.id','=','clinic_orders.clinic_id')
        ->leftJoin('product_name AS pn','pn.id','=','clinic_orders.product_id')
        ->where('clinic_orders.order_id',$request->order_id)
