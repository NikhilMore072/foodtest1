...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->action == 3)
        {
            ...
            $my_orders = ClinicOrders::select('clinic_orders.id', 'order_id', 'clinic_orders.created_at', 'total_price', 'order_status', 'received_remarks', DB::raw('GROUP_CONCAT(product_name.name) AS product_name'), DB::raw('GROUP_CONCAT(clinic_orders.product_qty) AS product_qty'))
            ->leftJoin('product_name', 'product_name.id', '=', 'clinic_orders.product_id')
            ->where('clinic_orders.user_id', Auth::user()->id)
            ->groupBy('clinic_orders.id', 'clinic_orders.order_id')
            ->orderBy('clinic_orders.id')
            ->get();
    ...
    public function report(){
        $data = TBLDispatch::leftJoin('use_product', 'tbl_dispatch.id', '=', 'use_product.tbl_dispatch_id')
            ->where(['tbl_dispatch.clinic_id' => Auth::user()->location_id])
            ->orderBy('tbl_dispatch.id', 'DESC')
            ->groupBy('tbl_dispatch.product_name','tbl_dispatch.manufacturer_name')
            ->selectRaw('tbl_dispatch.product_name, tbl_dispatch.category_name, tbl_dispatch.manufacturer_name, 
                SUM(tbl_dispatch.updated_qty) - COALESCE(SUM(use_product.use_qty), 0) as available_qty')
            ->get()
            ->toArray();
    
        return view('clinic_stock_report', compact('data'));
    }
    

public function all_report(Request $request)
{
    $userClinicLocations = Clinic_location::where('user_id', Auth::user()->id)->pluck('branch_name', 'id');

    $selectedClinicId = $request->input('clinic_id', 'central');

    $data = TBLDispatch::leftJoin('use_product', 'tbl_dispatch.id', '=', 'use_product.tbl_dispatch_id')
    ->where(['clinic_id' => $selectedClinicId])
    ->orderBy('tbl_dispatch.id', 'DESC')
    // ->groupBy('tbl_dispatch.product_name')
    ->groupBy('tbl_dispatch.product_name','tbl_dispatch.manufacturer_name')
    ->selectRaw('tbl_dispatch.product_name, tbl_dispatch.category_name, tbl_dispatch.manufacturer_name, 
        SUM(tbl_dispatch.updated_qty) - COALESCE(SUM(use_product.use_qty), 0) as total_quantity')
    ->get()
    ->toArray();
   
   

        $stocksdtl = DB::table('store')
        ...
        ->select(
            ...
            DB::raw('SUM(store.qty) AS total_quantity')
        )
        ->groupBy('ct.category_name', 'pn.name', 'mn.name')
        ->orderBy('product_name')
        ->get()
        ->toArray();

    return view('all_stock_report', compact('data', 'userClinicLocations', 'selectedClinicId', 'stocksdtl'));
}
